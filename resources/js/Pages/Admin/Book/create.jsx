      ...
      <form className="mx-3 p-5 flex gap-5" onSubmit={handleSubmit}>
        <div className="w-3/12 flex flex-col gap-5">
          <div className="p-5 border shadow-md rounded-xl flex flex-col gap-5">
            ...
            <div className="w-full flex justify-center relative">
              ...
              <div className="h-36 w-36 border shadow-md rounded-md items-center justify-center flex">
                <img src={preview ? preview : '/assets/image/image-thumbnail.png'} className="max-h-36" alt="" />
              </div>
            </div>
          ...
          <div className="p-5 border shadow-md rounded-xl flex flex-col gap-5">
            ...
            <ComboBox types={formattedTypes} setValue={(e, newValue) => setData('id_type', newValue.id)} />
        ...
        <div className="py-5 px-8 w-9/12 border shadow-md rounded-xl flex flex-col gap-5 h-fit">
          ...
          <div className="flex gap-5">
            <div className="flex flex-col gap-1 w-6/12">
              ...
              <input onChange={e => {setData('title', e.target.value)}} id="title" type="text" placeholder="Judul Buku" className="border border-gray-200 rounded-md px-4 py-2" />
            </div>
            <div className="flex flex-col gap-1 w-6/12">
              <label htmlFor="author">Penulis <span className="text-red-500">*</span></label>
              <input onChange={e => {setData('author', e.target.value)}} id="author" type="text" placeholder="Penulis" className="border border-gray-200 rounded-md px-4 py-2" />
            </div>
          </div>
          <div className="flex gap-5">
            <div className="flex flex-col gap-1 w-6/12">
              <label htmlFor="publisher">Penerbit <span className="text-red-500">*</span></label>
              <input onChange={e => {setData('publisher', e.target.value)}} id="publisher" type="text" placeholder="Penerbit" className="border border-gray-200 rounded-md px-4 py-2" />
            </div>
            <div className="flex flex-col gap-1 w-6/12">
              <label htmlFor="stock">Stok <span className="text-red-500">*</span></label>
              <input onChange={e => {setData('stock', e.target.value)}} id="stock" type="number" placeholder="Stock" className="border border-gray-200 rounded-md px-4 py-2" />
            </div>
          </div>
          <div className="flex gap-5">
            <div className="flex flex-col gap-1 w-6/12">
              <label htmlFor="location">Lokasi <span className="text-red-500">*</span></label>
              <input onChange={e => {setData('location', e.target.value)}} id="location" type="text" placeholder="Lokasi" className="border border-gray-200 rounded-md px-4 py-2" />
            </div>
          </div>
          <div className="flex gap-5 justify-end">
            <div>
              <Buttons title={'Batalkan'} variant={'outlined'} href={'/admin/buku'} />
            </div>
            <div>
